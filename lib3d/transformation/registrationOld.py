...
class register():
    ...
    def get_images(self, fno, ext='.jpg'):
        ...
        if rgb.shape[0] != depth.shape[0]:
            calib['camera_matrix']['data'] = self._resize_cam_camtrix(
                np.reshape(calib['camera_matrix']['data'], (3, 3)),
    ...
    def handle_node(self, fno):
        ...
        if rgb.shape[0] != depth.shape[0]: intrinsic = self._resize_cam_camtrix(intrinsic,
                                                                                scale=[depth.shape[1] / rgb.shape[1],
                                                                                       depth.shape[0] / rgb.shape[0],
                                                                                       1])
    ...
    def estimate_rgb_(self, src_img, targ_img):
        ...
        orb = cv2.ORB.create(scaleFactor=1.2, nlevels=8, edgeThreshold=31, firstLevel=0, WTA_K=2,
                             scoreType=cv2.ORB_HARRIS_SCORE, nfeatures=100, patchSize=31)
        ...
        for i_ in range(startf, stopf, skipf):
            ...
            if 0:
                ...
                reg_p2l = o3d.o3d.pipelines.registration.registration_icp(curr_pcd.voxel_down_sample(voxel_size),
                                                                          ...
                                                                          init=np.eye(4), estimation_method=method1,
                                                                          criteria=criteria1)
            ...
            if 1:
                ...
                reg_p2l = o3d.o3d.pipelines.registration.registration_colored_icp(
                    ...
                    copy.copy(finalpcd).voxel_down_sample(voxel_size), max_correspondence_distance=max_corres_dist,
                    init=np.eye(4), estimation_method=method, criteria=criteria1)
                # if len(np.asarray(reg_p2l.correspondence_set)):
                curr_pcd.transform(reg_p2l.transformation)
                correction = reg_p2l.transformation

            finalpcd += curr_pcd  # o3d.NormalViz([finalpcd])
