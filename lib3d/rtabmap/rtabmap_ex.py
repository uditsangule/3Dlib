...
def export(inputdb , filex='_',ascii=True , ba=False , optim=True , poses=True,mesh=False , formatpose=11):
    stat = 0
    return os.system(f"{path}/rtabmap-export {'--custom_optimization' if optim else ''} {'--ba' if ba else ''} \
                      --images_id --ground_normals_up {'--mesh' if mesh else ''} --prop_radius_factor 0.05 --poses --poses_format {formatpose} --outputdir {os.path.dirname(inputdb) + os.sep} --output {filex} {'--ascii' if ascii else ''} {inputdb}")
